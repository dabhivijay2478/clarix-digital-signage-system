...
import { formatScheduleTime } from '@/lib/signage-schedule'
...
function ScheduleTimeline({ slots, onDelete }: ScheduleTimelineProps) {
  ...
  const sortedSlots = [...slots].sort((a, b) => {
    const [aHour, aMinute] = a.start_time.split(':').map((part) => Number.parseInt(part, 10) || 0)
    const [bHour, bMinute] = b.start_time.split(':').map((part) => Number.parseInt(part, 10) || 0)
        ...
        <ScrollArea className="w-full">
          <div className="min-w-[840px] pb-4">
            <div className="mb-4 flex border-b border-border pb-2 pl-16 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
              {HOURS.map((hour) => (
                <div key={hour} className="min-w-8 flex-1 border-l border-border text-center first:border-l-0">
                  {formatScheduleTime(`${hour.toString().padStart(2, '0')}:00`)}
                </div>
              ))}
            </div>
              ...
              {DAYS.map((day) => (
                <div key={day} className="group/row relative flex min-h-11 items-center pl-16">
                  ...
                  <div className="relative h-8 flex-1 overflow-hidden rounded-md border border-border bg-muted/30">
                    ...
                    {sortedSlots.filter((slot) => slot.days_of_week.includes(day as never)).map((slot) => (

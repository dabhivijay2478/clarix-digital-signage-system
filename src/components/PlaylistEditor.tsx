...
export default function PlaylistEditor({ playlist, contentItems, onUpdateItems, onClose }: PlaylistEditorProps) {
  const [items, setItems] = useState<PlaylistItem[]>(() => (playlist.items || []).map((item) => ({ ...item, override_duration: null })))
  const totalDuration = items.reduce((acc, item) => acc + (contentItems.find((content) => content.id === item.content_id)?.duration_secs ?? 0), 0)
  ...
  return (
    <Card className="max-h-[85vh] overflow-hidden">
      <CardHeader className="flex flex-row items-start justify-between"><div><CardTitle>{playlist.name}</CardTitle><CardDescription>Edit playlist items. Playback uses each content item&apos;s default duration.</CardDescription></div><Badge variant="outline" className="font-mono">{totalDuration}s</Badge></CardHeader>
          ...
          <div className="space-y-2 pr-3">
            ...
            {items.map((item, index) => {
              ...
              return <Card key={`${item.content_id}-${index}`} className="bg-muted/20"><CardContent className="flex items-center gap-2 p-3">
                ...
                <div className="min-w-0 flex-1"><CardTitle className="truncate text-sm">{content.name}</CardTitle><CardDescription>{content.content_type} · {content.duration_secs}s default</CardDescription></div>
      ...
      <CardFooter className="justify-end gap-3 border-t border-border pt-4">{onClose && <Button variant="outline" onClick={onClose}>Cancel</Button>}<Button onClick={handleSave}>Save Changes</Button></CardFooter>
    </Card>
  )
}
